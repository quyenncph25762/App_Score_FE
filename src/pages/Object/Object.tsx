...
import { useAddObjectMutation, useFetchAllObjectQuery, useFetchOneObjectQuery, useRemoveObjectMutation, useUpdateObjectMutation } from '../../store/object/object.service';
...
const ObjectPage = () => {
    ...
    // lay 1 object
    const { data: getOneObject, isSuccess: isSuccessFetchOneObject, isError: isErrorFetchOneObject } = useFetchOneObjectQuery(idObject ? idObject : "")
    ...
    }, [idObject, isSuccessFetchOneObject, getOneObject, openUpdate])
    // modal
    // show modal them
    const showModal = () => {
        form.setFieldsValue({
    ...
    const onFinishUpdate = async (values: IObject) => {
        try {
            if (idObject) {
                const results = await onUpdate({ id: idObject, ...values })
                if (results.error) {
                    message.error(`Thêm thất bại , vui lòng thử lại!`);
                    return
                }
